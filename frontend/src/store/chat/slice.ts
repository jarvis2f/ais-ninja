...
import {ChatGpt, ChatPlugin, ChatsInfo} from '@/types'
  ...
  persist(
    (set, get) => ({
      ...
      setChatDataInfo: (id, messageId, info) =>
        set((state: ChatState) => {
          const plugin = info?.plugins?.[0];
          delete info?.plugins;
          const newChats = state.chats.map((item) => {
            if (item.id === id) {
              const newData = item.data.map((m) => {
                if (m.id === messageId) {
                  const { plugins: mPlugins = [] } = m;
                  if (plugin) {
                    const existingPlugin = mPlugins.find((p) => p.id === plugin.id);
                    if (existingPlugin) {
                      return {
                        ...m,
                        ...info,
              ...
              const chatData = { id: messageId, ...info, plugins: plugin ? [plugin] : [] } as ChatGpt
              return {
                ...item,
