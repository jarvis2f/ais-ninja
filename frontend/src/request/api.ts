...
export function postImagesGenerations(
	...
	params: {
		...
		height: number;
		quality?: number
	} | (GenerationRequestParams & { model: string }),
	headers?: { [p: string]: any },
	options?: { [p: string]: any }
) {
	return request.post<(Array<{ url: string }>) | (Array<TextToImageResponse>)>(
		`/api/u/${supplier}/images/generations`,
		{...params},
		headers,
		options
	)
...
// 卡密充值
export function postUseRedemptionCode(params: { key: string }) {
	return request.post('/api/u/use_redemption_code', params)
}
...
export function postPluginFunction(plugin_id: string, params: {
	name: string,
	description: string,
	parameters: string,
	script: string
...
export function putPluginFunction(plugin_id: string, id: string, params: {
	name: string,
	description: string,
	parameters: string,
	script: string
