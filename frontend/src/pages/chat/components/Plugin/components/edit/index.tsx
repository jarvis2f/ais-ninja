...
import {Button, Form, Input, Menu, message, Popconfirm, Space, Spin} from 'antd';
  ...
  return (
    <Spin spinning={spinning}>
      <div className={styles.plugin_edit}>
        ...
        >
          {editPlugin && (
            <ProForm form={pluginForm} onFinish={handlePluginFormFinish} onValuesChange={handlePluginFormValuesChange}>
              <ProForm.Item name="name" label={t('插件名称')} required rules={[{required: true}, {max: 64}]}>
                <Input placeholder="插件的名称" maxLength={64}/>
              </ProForm.Item>
              <ProForm.Item name="avatar" label={t('插件头像')} rules={[{max: 255}, {pattern: /^(https?|ftp):\/\//i}]}>
                <Input placeholder="插件头像Url" maxLength={255}/>
              </ProForm.Item>
              <ProForm.Item name="description" label={t('插件描述')} required rules={[{required: true}]}>
                <CodeInput height="400px" placeholder="描述这个插件可以干什么，支持Markdown" language="markdown"/>
              </ProForm.Item>
            </ProForm>
          )}
            ...
            <ProForm
              ...
              submitter={{
                render: (_, doms) => {
                  return [
                    ...doms,
                    <Button key="delete" type="primary" danger onClick={handleDeleteFunction}> {t('删除')} </Button>
                  ]
                }
              }}
            >
              ...
              <ProForm.Item name="description" label={t('方法描述')} required rules={[{required: true}, {max: 255}]}>
                <Input.TextArea placeholder="The description of what the function does." maxLength={255}/>
              </ProForm.Item>
              ...
              <ProForm.Item name="script" label={t('方法')} required rules={[{required: true}]} validateStatus="error">
